-- O total de Despesas Administrativas não fechava com a base porque a
-- classificação "DESP. ADM" dependia inteiramente do De/Para (conta_map):
-- qualquer lançamento de conta 3.4.01.* sem entrada em conta_map (ou
-- mapeada para outra categoria por engano) ficava fora do total. Agora
-- todo lançamento de conta 3.4.01.* é sempre considerado DESP. ADM,
-- independente do conta_map — a classificação de RECEITA BRUTA (usada
-- só para o % sobre receita) continua vindo do De/Para normalmente.
...
AS $function$
  ...
  enr AS (
    SELECT l.*,
      ...
      CASE
        WHEN COALESCE(l.vcodconta,'') = '3.4.01' OR COALESCE(l.vcodconta,'') LIKE '3.4.01.%' THEN 'DESP. ADM'
        ELSE COALESCE(am.categoria, 'NÃO MAPEADO')
      END AS cat
