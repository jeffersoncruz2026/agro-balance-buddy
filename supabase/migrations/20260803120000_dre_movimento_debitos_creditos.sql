-- Na DRE, o valor de cada linha passa a ser a movimentação do período
-- (débitos + créditos) em vez do saldo atual — que é o saldo acumulado,
-- adequado para o BP (contas patrimoniais), mas não para a DRE (contas
-- de resultado, medidas pelo movimento do período). O BP continua usando
-- saldo_atual, sem alteração.
